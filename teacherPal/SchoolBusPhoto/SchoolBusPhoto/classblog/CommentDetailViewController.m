...
//    else
//    {
//        if ([shareContent.sharePicArr count] > 0 && [shareContent.sharePicArr count]<=1)
//        {
    ...
    else if(method==deleteF)
    {
        if (code == 1) {
            ...
            if ([isDelUp isEqualToString:@"comment"]) {
                ...
                UIAlertView *alert=[[UIAlertView alloc]initWithTitle:nil message:NSLocalizedString(@"deleteSuccess", @"") delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"") otherButtonTitles:nil, nil];
                [alert show];
                [alert release];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        else
        {
            ...
            CGSize size = [names sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(250, 1000) lineBreakMode:NSLineBreakByWordWrapping];
...
//    NSLog(@"%@",self.comList);
    ...
    CGSize size = [content sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(200, 1000) lineBreakMode:NSLineBreakByWordWrapping];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0)
    {
        ...
        if (!self.upList || self.upList.count == 0)
        {
            for (UIImageView *obj in cell.contentView.subviews) {
                if (obj.tag >= 222 && obj.tag < 222 + 7)
                {
                    obj.frame = CGRectZero;
                }
            }
            cell.nameLab.frame = CGRectMake(50, 22, 250, 20);
            if (self.upList != nil) {
                cell.nameLab.text = NSLocalizedString(@"likenumber0", @"");
            }
            
            
        }
        else if (self.upList.count > 0 && self.upList.count <= 7)
        {
            for (UIImageView *obj in cell.contentView.subviews) {
                if (obj.tag >= 222 && obj.tag < 222 + self.upList.count)
                {
                    ...
                    if (dic != nil) {
                        ...
                        int i = obj.tag%222;
                        obj.frame = CGRectMake(50 + i * 35, 13 + 5, 30, 30);
                    }
                    
                    
                }
                else if (obj.tag >= 222 + self.upList.count && obj.tag < 222 + 7)
                {
                    obj.frame = CGRectZero;
                }
            }
        ...
        else if (self.upList.count > 7)
        {
            ...
            CGSize size = [names sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(250, 1000) lineBreakMode:NSLineBreakByWordWrapping];
            cell.nameLab.frame = CGRectMake(50, 20 + 5, size.width, size.height);
    ...
    else
    {
        ...
        if (!self.comList || self.comList.count == 0) {
            ...
            if (self.comList != nil) {
                cell.contentLabel.text = NSLocalizedString(@"commentnumber0", @"");
                cell.contentLabel.frame = CGRectMake(50, 12, 250, 20);
            }
...
//
    ...
    if (![isStudent isEqualToString:@"1"] && [senderId isEqualToString:user.teacher.teacherid]) {

        UIActionSheet *alert=[[UIActionSheet alloc]initWithTitle:NSLocalizedString(@"alert", @"") delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"") destructiveButtonTitle:NSLocalizedString(@"delete", @"") otherButtonTitles:nil, nil];
        alert.tag=333+indexPath.row;
    ...
    else
    {
        ...
        UIActionSheet *alert=[[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"") destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"replyComment", @""), nil];
    ...
    else if (actionSheet.tag == VIDEOTAG)
    {
        if (buttonIndex == 0)
        {
            ...
            else
            {
                ...
                UIAlertView *alert=[[UIAlertView alloc]initWithTitle:nil message:NSLocalizedString(@"nofinished", @"") delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"") otherButtonTitles:nil, nil];
                [alert show];
                [alert release];
...
//    WriteCommentsViewController *AA=[[WriteCommentsViewController alloc]init];
...
- (void)saveMovie:(UIGestureRecognizer *)gesture
{
    ...
    if(gesture.state == UIGestureRecognizerStateEnded)
    {
        ...
        UIActionSheet *action=[[UIActionSheet alloc]initWithTitle:NSLocalizedString(@"alert", @"") delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"") destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"baocunshipin", @""), nil];
        action.tag = VIDEOTAG;
